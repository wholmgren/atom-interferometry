...
Function PredictPhase2IFMmolAn(alpha, offset)
	...
	//Variable/G mass = kavgmass*amu2kg
	//Variable/G velocity = 2030
	//Variable/G sigma = 2030/23
	NVAR velocity, sigma
	...
	variable pospnts = 200
	variable posmax = .001
	
	Variable positionOffset = 0//.0001
	Make/D/O/N=(posPnts) position =  x/pospnts * posmax+positionOffset, predictedPhase
	//Make/D/O/N=(posPnts) position = .05 * x/1000/2+positionOffset, predictedPhase
...
Function gradphi()
	...
	Wave predictedphase, position
	
	FindLevel predictedphase, pi
	Variable xpipnt = v_levelx
	Variable xpi = position[v_levelx]
	print position[xpi]*1e6
	
	FindLevel position, xpi-25e-6
	Variable ifm1pos = v_levelx
	Variable ifm1phi = predictedphase[ifm1pos]
	print ifm1phi
	
	FindLevel position, xpi+25e-6
	Variable ifm2pos = v_levelx
	Variable ifm2phi = predictedphase[ifm2pos]
	print ifm2phi
	
	Variable dphi = ifm2phi-ifm1phi
	print dphi
	
	Duplicate/o predictedphase phigrad
	
	Differentiate phigrad /X=position
	phigrad/=1e6
	
	print phigrad[xpipnt]*50
end
...
Function CalculateConstantsHVin(HVin)
	Variable HVin
//	Variable/G Vwire = (6035 * HVmon)	 +0.7							// Applied electrode voltage [V]
