...
//	Will Holmgren, 11/1/11
//	Cronin Labs, Department of Physics, University of Arizona
//
//Instructions
//
//Run ChopperFitControlTable() to display a table of chopper model control and fit parameters
//
//Use CalculateChopperBestFit(w_coef, frequencyWave, series_name) to just model the choppers
//
//Use ChopperDCphase(series_name, OnekTwok) to extract the chopper DC phase data from FringeFit output
//Use GoFitChopperDCphase(series_name) to fit the data chopper DC phase data
//
//Use ExtractChopperData(series_name, onekTwok, startPntsToKill, stopPntsToKill) to extract the chopper frequency data from FringeFit output
//Use GoFitPhaseChoppers(series_name, onektwok) to fit the extracted chopper frequency data
//
//Here's the rough idea behind the code: The modeling wrapper function CalculateChopperBestFit() and the fit wrapper function GoFitPhaseChoppers() both
//start by calling a helper function MakeChopperFitHelperWaves() (or some variant of it). The "helper waves" are 3D waves corresponding to
//the differential phase shifts acquired by atoms with different: 1) start times 2) velocities 3) chopping frequencies. Then, the wrapper functions
//call PhaseChoppersFit() (or some variant of it) either once for modeling or many times as part of the fit routine. This function essentially
//takes an input velocity distribution, performs the velocity distribution weighted numerical integration of the different phase shifts calculated by the 
//MakeChopperFitHelperWaves() function, and outputs the resulting contrast as a function of chopping frequency. 
//
//Additional features/functions include: Sagnac phase shifts, difference between vel_pi and vel_0, graphing/display, beam width modeling


//	Wrapper function for calculating the chopper scans given a wave of parameters, frequencies, and a string to append to the results
Function CalculateChopperBestFit(w_coef, frequencyWave, series_name)
	Wave w_coef, frequencyWave
	String series_name
	
	Variable bestFitPnts
//	bestFitPnts = numpnts(frequencyWave)//200
	bestFitPnts = 200
//	NVAR fpnts
//	fpnts = bestFitPnts	
...
//	MakeChopperFitHelperWaves3chop(freqFit)
//	MakeChopperFitHelperWaves4chop(freqFit)
//	MakeChopperFitHelperWaves8chop(freqFit)
...
Function FindVpi(FlowV, vRatio, phiMeasured)
	...
	// fofv = (2*x0*s(v) - s(v)^2) / ( (b^2-x0^2) * (b^2-(x0-s(v))^2) )
...
Function PhiRooter(w,x)
	...
	Variable phi = atan2(impart,repart)
	Variable phiDiff = phi-phiMeasured
	
	Variable phiUnwrapped = phi + 2*pi*floor((phiFlowV-phi)/(2*pi))
	
	return phiUnWrapped-phiMeasured
End
...
Function FindEmpiricalFactor(FlowV, vRatio, phiMeasured, c1orc2)
	Variable FlowV, vRatio, phiMeasured, c1orc2
	
	DFREF originalDFR = GetDataFolderDFR()
	SetDataFolder root:ChopperCalculations
	
	Wave velwave, velprobs, beampos//, w_coef, InitialGuesses
	
	Variable sigV = FlowV/vratio
	Variable velnorm
	
	//NVAR centerpos, b//, phiMeasured
	//NVAR chop1centerPos, chop2centerPos
	//NVAR k1Gc1, k1G2G, k3Gc2
	
	// import global variables for the phase shift along the center line with the flow velocity
	// at this point the phase shifts are unnormalized
	//NVAR pos_c1_p1_center_FlowV, pos_c1_m1_center_FlowV, pos_c2_p1m1_center_FlowV, pos_c2_0p1_center_FlowV, pos_c2_m1p1_center_FlowV, pos_c2_0m1_center_FlowV
	//NVAR phase_c1_IFMp1_center_FlowV, phase_c1_IFMm1_center_FlowV, phase_c2_IFMp1_center_FlowV, phase_c2_IFMm1_center_FlowV
	NVAR phase_c1_IFMavg_center_FlowV, phase_c2_IFMavg_center_FlowV
	
	//	use a switch statement to figure out which waves (the c1 or the c2 waves) to point our computations to
	Variable phase_center_FlowV
	switch(c1orc2)
		case 1:
			phase_center_FlowV = phase_c1_IFMavg_center_FlowV		// unnormalized phase shift along beam center for the flow velocity. used for unwrapping the calculated phase shift
			break
		case 2:
			phase_center_FlowV = phase_c2_IFMavg_center_FlowV
			break
		default:
			abort "is it c1 or c2? FindEmpericalFactor()"
	endswitch
	
	Make/d/o PhiRooterCoef = {phiMeasured, phase_center_FlowV, c1orc2}
	
	FindRoots/Q PhiRooterEmpirical, PhiRooterCoef		// PhiRooterEmperical is the function that we want to find the roots of. PhiRooterCoef is a wave that we can use to pass parameters to that function
	
	Variable/g theRoot = V_root
	Variable/G phase_center_FlowV_norm = phase_center_FlowV*theroot
	print theroot
	print phase_center_FlowV_norm
	//FofVc = FofV*theRoot
	//FindLevel/Q FofVc, phiMeasured
	//Variable/g vpi = v_levelx
	//Variable/g vpipercent = (vpi-flowv)/flowv*100
...
Function PhiRooterEmpirical(w,x)
	Wave w
	Variable x
	
	wave velprobs, re_phase_int_emp, im_phase_int_emp
	
	Variable phiMeasured = w[0]
	Variable phiUnnormalized = w[1]
	Variable c1orc2 = w[2]
	
	NVAR vPnts, posPnts
	
	Variable phi_center_FlowV_Norm = phiUnnormalized*x
	
	//	use a switch statement to figure out what waves (the c1 or the c2 waves) to point our computations to
	switch(c1orc2)
		case 1 :
			Wave phase_p1_v_pos_int = phase_c1_IFMp1_v_pos
			Wave phase_m1_v_pos_int = phase_c1_IFMm1_v_pos
			Wave posProbs = chop1PosProb
			break
		case 2 :
			Wave phase_p1_v_pos_int = phase_c2_IFMp1_v_pos
			Wave phase_m1_v_pos_int = phase_c2_IFMm1_v_pos
			Wave posProbs = chop2posProb
			break
		default :
			abort "is it c1 or c2? PhiRooterEmperical()"
	endswitch
	 
	// 	calculate the real and imaginary components and weight them with the position and velocity probilities
	re_phase_int_emp = velprobs[p]*posProbs[q]*cos(x*phase_p1_v_pos_int)							
	im_phase_int_emp = velprobs[p]*posProbs[q]*sin(x*phase_p1_v_pos_int)
	
	//	integrate the real and imaginary components over velocity (DIM=0) and position (DIM=1)
//	Integrate/DIM=0 re_phase_int_emp
//	Integrate/DIM=1 re_phase_int_emp
//	Integrate/DIM=0 im_phase_int_emp
//	Integrate/DIM=1 im_phase_int_emp
	
	Integrate/DIM=0/METH=1 re_phase_int_emp
	Integrate/DIM=1/METH=1 re_phase_int_emp
	Integrate/DIM=0/METH=1 im_phase_int_emp
	Integrate/DIM=1/METH=1 im_phase_int_emp
	
//	Integrate/DIM=0/P re_phase_int_emp
//	Integrate/DIM=1/P re_phase_int_emp
//	Integrate/DIM=0/P im_phase_int_emp
//	Integrate/DIM=1/P im_phase_int_emp
//	Variable velProbsDelta = DimDelta(velprobs, 0)
//	re_phase_int_emp/=3*75*velProbsDelta
//	im_phase_int_emp/=3*75*velProbsDelta
	
	Variable conCalc = sqrt(im_phase_int_emp[vPnts][posPnts]^2+re_phase_int_emp[vPnts][posPnts]^2); print conCalc
	Variable phiCalc = atan2(im_phase_int_emp[vPnts][posPnts],re_phase_int_emp[vPnts][posPnts])		// calculated phi, but could be wrong by n*2*pi since the arctangent is only defined modulo 2*pi
	Variable phiDiffUnwrap = phiCalc -phi_center_FlowV_Norm											// difference between calculated phi and phi for simply the beam centerline and flow v
	
	Variable phiUnwrapped = phiCalc - 2*pi*round(phiDiffUnwrap/(2*pi))								// undo the potential n*2*pi error
	
	Variable phiCalcMeasDiff = phiUnWrapped-phiMeasured													// the difference between the calculated phase with this normalization constant and the measured phase
	//phiCalcMeasDiff = phiCalc-phiMeasured
	
	return phiCalcMeasDiff																					// we want this to be 0
End
...
Function MakeChopperFitHelperWaves3chop(freq)
	Wave freq
	...
	Make/D/O/N=(fPnts, vPnts, tPnts) phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, sagPhaseWave//,ttest
	Make/D/O/N=(fPnts, vPnts) phi_fv_re, phi_fv_im
	Make/D/O/N=(fPnts) phi_f_re, phi_f_im, phi_f, con_f
	//SetScale/I x minChopFreq, maxChopFreq, "Hz", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_f_re, phi_f_im, phi_f, contrast_f, ttest, phi_fvt_diff
	SetScale/I y minVel, maxVel, "m/s", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, sagPhaseWave//,ttest
	SetScale/P z 0, 1/tpnts, "t*f", phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, sagPhaseWave
	...
	MultiThread foftsqrdexpChop1 = foftsqrdexp( z/freq[p] , freq[p]) / y^2
	MultiThread foftsqrdexpChop2 = foftsqrdexp( z/freq[p] + chopL/2/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop3 = foftsqrdexp( z/freq[p] + chopL/y, freq[p]) / y^2
...
Function MakeChopperFitHelperWaves4chop(freq)
	Wave freq
	...
	Make/D/O/N=(fPnts, vPnts, tPnts) phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4, sagPhaseWave//,ttest
	Make/D/O/N=(fPnts, vPnts) phi_fv_re, phi_fv_im
	Make/D/O/N=(fPnts) phi_f_re, phi_f_im, phi_f, con_f
	//SetScale/I x minChopFreq, maxChopFreq, "Hz", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_f_re, phi_f_im, phi_f, contrast_f, ttest, phi_fvt_diff
	SetScale/I y minVel, maxVel, "m/s", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4, sagPhaseWave//,ttest
	SetScale/P z 0, 1/tpnts, "t*f", phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4, sagPhaseWave
	...
	MultiThread foftsqrdexpChop1 = foftsqrdexp( z/freq[p] , freq[p]) / y^2
	MultiThread foftsqrdexpChop2 = foftsqrdexp( z/freq[p] + 1/3*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop3 = foftsqrdexp( z/freq[p] + 2/3*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop4 = foftsqrdexp( z/freq[p] + chopL/y, freq[p]) / y^2
...
Function MakeChopperFitHelperWaves8chop(freq)
	Wave freq
	...
	Make/D/O/N=(fPnts, vPnts, tPnts) phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4,  foftsqrdexpChop5, foftsqrdexpChop6, foftsqrdexpChop7, foftsqrdexpChop8,  sagPhaseWave//,ttest
	Make/D/O/N=(fPnts, vPnts) phi_fv_re, phi_fv_im
	Make/D/O/N=(fPnts) phi_f_re, phi_f_im, phi_f, con_f
	//SetScale/I x minChopFreq, maxChopFreq, "Hz", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_f_re, phi_f_im, phi_f, contrast_f, ttest, phi_fvt_diff
	SetScale/I y minVel, maxVel, "m/s", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4,  foftsqrdexpChop5, foftsqrdexpChop6, foftsqrdexpChop7, foftsqrdexpChop8, sagPhaseWave//,ttest
	SetScale/P z 0, 1/tpnts, "t*f", phi_fvt_re, phi_fvt_im, phi_fvt_diff, foftsqrdexpChop1, foftsqrdexpChop2, foftsqrdexpChop3, foftsqrdexpChop4,  foftsqrdexpChop5, foftsqrdexpChop6, foftsqrdexpChop7, foftsqrdexpChop8, sagPhaseWave
	...
	MultiThread foftsqrdexpChop1 = foftsqrdexp( z/freq[p] , freq[p]) / y^2
	MultiThread foftsqrdexpChop2 = foftsqrdexp( z/freq[p] + 1/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop3 = foftsqrdexp( z/freq[p] + 2/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop4 = foftsqrdexp( z/freq[p] + 3/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop5 = foftsqrdexp( z/freq[p] + 4/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop6 = foftsqrdexp( z/freq[p] + 5/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop7 = foftsqrdexp( z/freq[p] + 6/7*chopL/y, freq[p]) / y^2
	MultiThread foftsqrdexpChop8 = foftsqrdexp( z/freq[p] + 7/7*chopL/y, freq[p]) / y^2
...
Function MakeChopperFitHelperWavesModern(freq)
	Wave freq
	...
	DFREF originalDFR = GetDataFolderDFR()
	NewDataFolder/O/s ChopperCalculations


	// frequency points to do the calculation for
	Variable/G fPntsGlobal = numpnts(freq)
	Variable fPnts = numpnts(freq)

	// velocity, time, and position points to integrate over
	Variable/G vPnts = 75	//was 100
	Variable nSigmas = 5
	
	Variable/G tpnts = 300	//was 500
	
	Variable/G posPnts = 5
	
	//	beam width and position parameters
	//Variable/G centerPos = .8e-3					// distance from ground plane
	//Variable beamWidth = 10e-6					// full width
	//Variable minPos = centerPos-beamWidth/2
	//Variable maxPos = centerPos+beamWidth/2

	Variable/G chop1centerPos = 0.8e-3				// distance from ground plane to undiffracted path (0th 1G order) at c1
	Variable/G chop2centerPos = 0.8e-3				// distance from ground plane to undiffracted path (0th 1G and 0th 2G) at c2 (note that the undiffracted path at c2 does not play a part in the interference signal)
	
	Variable/G chop1pathWidth = 200e-6				// path width at c1. same as beam width w/o diffraction grating
	Variable/G chop2pathWidth = 200e-6				// path width at c2. 			"	 "	
	Variable/G chop1minPos = chop1centerPos-chop1pathWidth/2	
	Variable/G chop1maxPos = chop1centerPos+chop1pathWidth/2	
	Variable/G chop2minPos = chop2centerPos-chop2pathWidth/2	
	Variable/G chop2maxPos = chop2centerPos+chop2pathWidth/2	
			
	Variable c11gdist = 11*25.4e-3
	Variable c23gdist = 13*25.43-3
	Variable OneG2Gdist = .941
	

	//	chopper geometry parameters
	Variable a = 1e-3								// ground plane to wire dist
	Variable radius = 0.785e-3						// wire radius
	Variable/G b = a*sqrt(1+2*radius/a)


	//	set up the velocity range to integrate over
	Variable minVel = FlowV*(1-nSigmas/15)//; minVel = 0
	Variable maxVel = FlowV*(1+nSigmas/15)//; maxVel = 5000
	Variable velStep = (maxVel-minVel)/vPnts
	
	Make/D/O/N=(vPnts) velwave, velprobs, pos_c1_p1, pos_c1_m1, pos_c2_p1m1, pos_c2_0p1, pos_c2_m1p1, pos_c2_0m1
	SetScale/I x minVel,maxVel,"m/s", velwave, velprobs, pos_c1_p1, pos_c1_m1, pos_c2_p1m1, pos_c2_0p1, pos_c2_m1p1, pos_c2_0m1
	velwave = x															// a wave of velocities
	Variable velNorm
	Variable vCubed = 0
	if(vCubed)
		Velprobs = exp(-vratio^2/2*(x/FlowV-1)^2)*x^3
	...
	//	Make waves for handling the beam position
	Make/D/O/N=(posPnts) beamPos, beamChopProfile, deltaPos, chop1beamPos, chop2beamPos, chop1posProb, chop2posProb
	Make/D/O/N=(posPnts) chop1beamPos, chop2beamPos, chop1posProb, chop2posProb
	//SetScale/i x minPos, maxPos, "m", beamPos
	//beampos= numpnts(beampos) == 1 ? centerpos : x
	//beamChopProfile = 1/numpnts(beampos)
	chop1posProb = 1
	chop2posProb = 1
	Variable positionPhase1, positionPhase2, positionFactor
	//Variable diffPos = centerPos+10e-6
	//Variable c = (b^2-centerPos^2)*(b^2-diffPos^2)/(centerPos^2-diffPos^2)
	
	//	calculate the path separation constants. path separation = k / v
	Variable thisMass = kavgmass
	Variable/G k1Gc1 = h*L1Gc1/(thisMass*amu2kg)/gratingPeriod
	Variable/G k1G2G = h*L1G2G/(thisMass*amu2kg)/gratingPeriod
	Variable/G k3Gc2 = h*L3Gc2/(thisMass*amu2kg)/gratingPeriod
	
	// positions of the diffracted beam assuming that the undiffracted beam travels straight along the centerPos position
	pos_c1_p1 = chop1centerPos + k1Gc1/x	
	pos_c1_m1 = chop1centerPos - k1Gc1/x
	pos_c2_p1m1 = chop2centerPos + k1G2G/x
	pos_c2_0p1 = chop2centerPos + k1G2G/x - k3Gc2/x
	pos_c2_m1p1 = chop2centerPos - k1G2G/x
	pos_c2_0m1 = chop2centerPos - k1G2G/x + k3Gc2/x
	
	Duplicate/O pos_c1_p1 phase_c1_IFMp1, phase_c2_IFMp1, phase_c1_IFMm1, phase_c2_IFMm1
	
	phase_c1_IFMp1 = 1/(b^2-pos_c1_p1^2) - 1/(b^2-chop1centerPos^2)
	phase_c1_IFMm1 = 1/(b^2-chop1centerPos^2) - 1/(b^2-pos_c1_m1^2)
	phase_c2_IFMp1 = 1/(b^2-pos_c2_p1m1^2) - 1/(b^2-pos_c2_0p1^2)
	phase_c2_IFMm1 = 1/(b^2-pos_c2_0m1^2) - 1/(b^2-pos_c2_m1p1^2)
	
	
	//	Make variables for analyzing the DC phase measurements. Unnormalized phase for the flow velocity and along the beam center
	Variable/G pos_c1_p1_center_FlowV = chop1centerPos + k1Gc1/FlowV	
	Variable/G pos_c1_m1_center_FlowV = chop1centerPos - k1Gc1/FlowV
	Variable/G pos_c2_p1m1_center_FlowV = chop2centerPos + k1G2G/FlowV
	Variable/G pos_c2_0p1_center_FlowV = chop2centerPos + k1G2G/FlowV - k3Gc2/FlowV
	Variable/G pos_c2_m1p1_center_FlowV = chop2centerPos - k1G2G/FlowV
	Variable/G pos_c2_0m1_center_FlowV = chop2centerPos - k1G2G/FlowV + k3Gc2/FlowV
	
	Variable/G phase_c1_IFMp1_center_FlowV = 1/(b^2-pos_c1_p1_center_FlowV^2) - 1/(b^2-chop1centerPos^2)
	Variable/G phase_c1_IFMm1_center_FlowV = 1/(b^2-chop1centerPos^2) - 1/(b^2-pos_c1_m1_center_FlowV^2)
	Variable/G phase_c1_IFMavg_center_FlowV = (phase_c1_IFMp1_center_FlowV + phase_c1_IFMm1_center_FlowV)/2
	Variable/G phase_c2_IFMp1_center_FlowV = 1/(b^2-pos_c2_p1m1_center_FlowV^2) - 1/(b^2-pos_c2_0p1_center_FlowV^2)
	Variable/G phase_c2_IFMm1_center_FlowV = 1/(b^2-pos_c2_0m1_center_FlowV^2) - 1/(b^2-pos_c2_m1p1_center_FlowV^2)
	Variable/G phase_c2_IFMavg_center_FlowV = (phase_c2_IFMp1_center_FlowV + phase_c2_IFMm1_center_FlowV)/2
	
	//	Make waves for analyzing the DC phase measurements. Unnormalized phase as a function of velocity and beam position
	Make/D/O/N=(vPnts, posPnts) phase_c1_IFMp1_v_pos, phase_c2_IFMp1_v_pos, phase_c1_IFMm1_v_pos, phase_c2_IFMm1_v_pos, pos_c1_p1_v, pos_c1_m1_v, pos_c2_p1m1_v, pos_c2_0p1_v, pos_c2_m1p1_v, pos_c2_0m1_v, pos_c1_0_v
	SetScale/I x minVel,maxVel,"m/s", pos_c1_0_v, pos_c1_p1_v, pos_c1_m1_v, pos_c2_p1m1_v, pos_c2_0p1_v, pos_c2_m1p1_v, pos_c2_0m1_v, phase_c1_IFMp1_v_pos, phase_c2_IFMp1_v_pos, phase_c1_IFMm1_v_pos, phase_c2_IFMm1_v_pos
	SetScale/I y chop1minPos, chop1maxPos, "m", pos_c1_0_v, pos_c1_p1_v, pos_c1_m1_v, phase_c1_IFMp1_v_pos,phase_c1_IFMm1_v_pos
	SetScale/I x chop1minPos, chop1maxPos, "m", chop1posProb, chop1beamPos
	SetScale/I y chop2minPos, chop2maxPos, "m", pos_c2_p1m1_v, pos_c2_0p1_v, pos_c2_m1p1_v, pos_c2_0m1_v,  phase_c2_IFMp1_v_pos , phase_c2_IFMm1_v_pos
	SetScale/I x chop2minPos, chop2maxPos, "m", chop2posProb, chop2beamPos
	pos_c1_0_v = y
	pos_c1_p1_v = y + k1Gc1/x	
	pos_c1_m1_v = y - k1Gc1/x
	pos_c2_p1m1_v = y + k1G2G/x
	pos_c2_0p1_v = y + k1G2G/x - k3Gc2/x
	pos_c2_m1p1_v = y- k1G2G/x
	pos_c2_0m1_v =y - k1G2G/x + k3Gc2/x
	phase_c1_IFMp1_v_pos = 1/(b^2-pos_c1_p1_v^2) - 1/(b^2-pos_c1_0_v^2)
	phase_c1_IFMm1_v_pos = 1/(b^2-pos_c1_0_v^2) - 1/(b^2-pos_c1_m1_v^2)
	phase_c2_IFMp1_v_pos = 1/(b^2-pos_c2_p1m1_v^2) - 1/(b^2-pos_c2_0p1_v^2)
	phase_c2_IFMm1_v_pos = 1/(b^2-pos_c2_0m1_v^2) - 1/(b^2-pos_c2_m1p1_v^2)
	
	chop1beamPos = x
	chop2beamPos = x
	
	Variable chopprobnorm
	chopProbNorm = area(chop1posProb)
	chop1posProb/= chopProbNorm
	chopProbNorm = area(chop2posProb)
	chop2posProb/= chopProbNorm
	
	Duplicate/O phase_c1_IFMp1_v_pos re_phase_int_emp, im_phase_int_emp
	
	
	//	Make waves to hold the calculation results
	Make/D/O/N=(fPnts, vPnts, tPnts) phi_fvt_re, phi_fvt_im, phi_fvt_diff, sagPhaseWave
	Make/D/O/N=(fPnts, vPnts) phi_fv_re, phi_fv_im
	Make/D/O/N=(fPnts) phi_f_re, phi_f_im, phi_f, con_f
	
	//	Set the velocity and time scales for the calculation results waves
	SetScale/I y minVel, maxVel, "m/s", phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_fvt_diff, sagPhaseWave
	SetScale/P z 0, 1/tpnts, "t*f", phi_fvt_re, phi_fvt_im, phi_fvt_diff, sagPhaseWave
	
	String thisBeamPosStr
	
	Make/O/N=(posPnts)/Wave waves_c1_IFMp1, waves_c1_IFMm1, waves_c2_IFMp1, waves_c2_IFMm1
	
	Variable thisbeamposc1, thisbeamposc2
	
	String phaseWN_c1_IFMp1, phaseWN_c2_IFMp1,phaseWN_c1_IFMm1, phaseWN_c2_IFMm1		// strings to hold phase wave names for each chopper and position
	Variable i,j
	Variable thisBeamPos
	For(i=0; i<posPnts; i+=1)	// this loop will make phase waves for each interferometer at each chopper and each beam position. 
		thisBeamPosC1 = chop1beamPos[i]
		thisBeamPosC2 = chop2beamPos[i]
		
		// calculate the beam path positions as a function of velocity
		pos_c1_p1 = thisBeamPosC1 + k1Gc1/x	
		pos_c1_m1 = thisBeamPosC1 - k1Gc1/x
		pos_c2_p1m1 = thisBeamPosC2 + k1G2G/x
		pos_c2_0p1 = thisBeamPosC2 + k1G2G/x - k3Gc2/x
		pos_c2_m1p1 = thisBeamPosC2 - k1G2G/x
		pos_c2_0m1 = thisBeamPosC2 - k1G2G/x + k3Gc2/x
	
		// calculate the position dependence of the phase shift (these expressions include 1 factor of 1/v)
		phase_c1_IFMp1 = 1/(b^2-pos_c1_p1^2) - 1/(b^2-thisBeamPos^2)
		phase_c1_IFMm1 = 1/(b^2-thisBeamPos^2) - 1/(b^2-pos_c1_m1^2)
		phase_c2_IFMp1 = 1/(b^2-pos_c2_p1m1^2) - 1/(b^2-pos_c2_0p1^2)
		phase_c2_IFMm1 = 1/(b^2-pos_c2_0m1^2) - 1/(b^2-pos_c2_m1p1^2)
		
		// make the wave names
		thisBeamPosStr = num2str(round(thisBeamPos*1e6))
		phaseWN_c1_IFMp1 = "phaseNorm_c1_IFMp1_pos"+thisBeamPosStr
		phaseWN_c2_IFMp1 = "phaseNorm_c2_IFMp1_pos"+thisBeamPosStr	
		phaseWN_c1_IFMm1 = "phaseNorm_c1_IFMm1_pos"+thisBeamPosStr	
		phaseWN_c2_IFMm1 = "phaseNorm_c2_IFMm1_pos"+thisBeamPosStr
	
		Make/D/O/N=(fPnts, vPnts, tPnts) $phaseWN_c1_IFMp1, $phaseWN_c2_IFMp1, $phaseWN_c1_IFMm1, $phaseWN_c2_IFMm1
		Wave loopPhase_c1_IFMp1 = $phaseWN_c1_IFMp1
		Wave loopPhase_c2_IFMp1 = $phaseWN_c2_IFMp1
		Wave loopPhase_c1_IFMm1 = $phaseWN_c1_IFMm1
		Wave loopPhase_c2_IFMm1 = $phaseWN_c2_IFMm1
		
		// assign wave references to references wave. will be used to access waves in fitting calculation
		waves_c1_IFMp1[i] = loopPhase_c1_IFMp1
		waves_c1_IFMm1[i] = loopPhase_c1_IFMm1
		waves_c2_IFMp1[i] = loopPhase_c2_IFMp1
		waves_c2_IFMm1[i] = loopPhase_c2_IFMm1
		
		SetScale/I y minVel, maxVel, "m/s", loopPhase_c1_IFMp1, loopPhase_c2_IFMp1, loopPhase_c1_IFMm1, loopPhase_c2_IFMm1
		SetScale/P z 0, 1/tpnts, "t*f", loopPhase_c1_IFMp1, loopPhase_c2_IFMp1, loopPhase_c1_IFMm1, loopPhase_c2_IFMm1
		
		MultiThread loopPhase_c1_IFMp1 = foftsqrdexp( z/freq[p] , freq[p]) / y * phase_c1_IFMp1[q]	// only need y instead of y^2 because the velocity-position dependence is contained in phase_c1_IFMp1
		MultiThread loopPhase_c1_IFMm1 = foftsqrdexp( z/freq[p] , freq[p]) / y * phase_c1_IFMm1[q]	
		MultiThread loopPhase_c2_IFMp1 = foftsqrdexp( z/freq[p] + chopL/y, freq[p]) / y * phase_c2_IFMp1[q]
		MultiThread loopPhase_c2_IFMm1 = foftsqrdexp( z/freq[p] + chopL/y , freq[p]) / y * phase_c2_IFMm1[q]	
	EndFor
	
	
	Duplicate/O phi_f_re, phi_f_re_temp, phi_f_im_temp				// what are the temp waves for???
	phi_f_re=0;phi_f_im=0;phi_f_re_temp=0; phi_f_im_temp=0
...
Function PhaseChoppersModern(pw, yw, freq) : FitFunc
	Wave pw, yw, freq
	
	// pw0 = flow velocity
	// pw1 = vratio
	// pw2 = contrast normalization constant
	// pw3 = phi12err
	// pw4 = delta phase
	...
	Wave Velprobs, phi_fvt_diff, beampos, beamChopProfile, phi_fvt_re, phi_fvt_im, phi_fv_re, phi_fv_im, phi_f_re, phi_f_im, phi_f, con_f, phi_f_re_temp, phi_f_im_temp, phiSagGravChop
	NVAR b, centerpos, tPnts, vPnts, fPntsGlobal, AvgSagGravPhi, AvgSagGravCon
	
	// get references for waves that hold references to the phase data for each beam position for a given chopper and interferometer
	Wave/Wave waves_c1_IFMp1, waves_c1_IFMm1, waves_c2_IFMp1, waves_c2_IFMm1
	
	// set the type of velocity distribution to fit to
	Variable vCubed = 0
	if(vCubed)
		Velprobs = exp(-vratio^2/2*(x/FlowV-1)^2)*x^3
		VelNorm = sqrt(2*pi)*sigv*FlowV*(FlowV^2+3*sigv^2)
	...
	// initialize the calculation waves
	phi_fvt_diff=0
	phi_fvt_re=0
	phi_fvt_im=0
	phi_f_re = 0
	...
	Variable factor1 = FindEmpiricalFactor(FlowV, vRatio, phi1,1)	// 
	Variable factor2 = FindEmpiricalFactor(FlowV, vRatio, phi2,2)
	
	//	Variable flowvPhi12err = flowv^2*phi12avg
	
	Variable sagSign = 1
	if(numpnts(beampos)==0 || numpnts(beampos)==1)
		MatrixOP/O/S phi_fvt_diff = factor1*waves_c1_IFMp1[0] - factor2*waves_c2_IFMp1[0] + sagSign*phiSagGravChop		// Still 5 times faster than a multithreaded wave assignment
...
Function PhaseChoppers2IFMfitv3(pw, yw, freq) : FitFunc
	...
	if(numpnts(beampos)==0 || numpnts(beampos)==1)
		...
		//MatrixOP/O/S phi_fvt_diff = phi1VpiSqrd*foftsqrdexpChop1 - phi2VpiSqrd*foftsqrdexpChop2// + sagSign*phiSagGravChop
		//Wave foftsqrdexpChop3; MatrixOP/O/S phi_fvt_diff = phi1VpiSqrd*foftsqrdexpChop1 - phi2VpiSqrd*foftsqrdexpChop2 + phi2VpiSqrd*foftsqrdexpChop3
		//Wave foftsqrdexpChop3, foftsqrdexpChop4; MatrixOP/O/S phi_fvt_diff = phi1VpiSqrd*foftsqrdexpChop1 - phi2VpiSqrd*foftsqrdexpChop2 + phi1VpiSqrd*foftsqrdexpChop3 - phi2VpiSqrd*foftsqrdexpChop4
		//Wave foftsqrdexpChop3, foftsqrdexpChop4, foftsqrdexpChop5, foftsqrdexpChop6, foftsqrdexpChop7, foftsqrdexpChop8; MatrixOP/O/S phi_fvt_diff = phi1VpiSqrd*foftsqrdexpChop1 - phi2VpiSqrd*foftsqrdexpChop2 + phi1VpiSqrd*foftsqrdexpChop3 - phi2VpiSqrd*foftsqrdexpChop4 + phi1VpiSqrd*foftsqrdexpChop5 - phi2VpiSqrd*foftsqrdexpChop6 + phi1VpiSqrd*foftsqrdexpChop7 - phi2VpiSqrd*foftsqrdexpChop8
		MatrixOP/O/S phi_fvt_re = cos(	phi_fvt_diff	)
		MatrixOP/O/S phi_fvt_im = sin(	phi_fvt_diff	)
	else
		//ttime= stopMSTimer(-2)
...
Function CleanupChopperFitWaves()
	...
	KillWaves/z voltageon, phi_fvt_diffM, phi_fvt_diff2, foftsqrdexpChop3, foftsqrdexpChop4, foftsqrdexpChop5, foftsqrdexpChop6, foftsqrdexpChop7, foftsqrdexpChop8
End



Function SagnacAndGravityPhaseChop()
	Wave velwave, velprobs, sagPhaseWave
...
//	Duplicate/O foftsqrdexpChop1 phiSagGravChop
	Duplicate/O sagPhaseWave phiSagGravChop
