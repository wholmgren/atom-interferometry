...
#pragma version = 2.0		// 2.0 = 2 IFM model, 0 position = 50% counts
...
//	Prompt KillEclipseFilesLoc, "Kill Eclipse Files: yes (1) no (0) "
	...
	DoPrompt CurrentDataFolder+ " General Parameters", velLoc, sigLoc, InitialGradEMotorPositionLoc, FinalGradEMotorPositionLoc, PositionLoc, AtomLoc, LastEclipseLoc, LastFileLoc, EclipseFilesToKillStrLoc, MolFracLoc
...
Function PredictPhaseVoltage(alpha)
	...
	Make/O/N=20 HVmonWave = .05 * p, predictedPhase
...
Function PredictPhaseVoltage2IFM(alpha)
	Variable alpha
	//Variable position
	...
	xx[][][] = ((q * x_step) - (.5*num_x * x_step))
	MatrixOP/O xxsqr = magsqr(xx)	
	Make/o/n=(num_HV) IFMphase IFMprobM IFMprobP ContrastM ContrastP						//Total phase shift of monochrom. beam; phase shift of polychromatic beam
	...
	Calc_EfieldVoltage2IFM(position)							// calculate E-field at each data point
	
	CalcPhiOverPolVoltage2IFM()										// calculate phase/polarizability
	
	PhiVoltage2IFM(paramWave, predictedPhase, Vwire1d)
...
Function PhaseFitVoltage2IFM(VwireWave, PhaseWave, ErrorWave)
	Wave VwireWave, PhaseWave, ErrorWave
	...
	Make/D/O Epsilonwave={1e-1}			// Amount to vary the fit parameter by in each iteration	
	FuncFit/N/M=2/TBOX=768 PhiVoltage2IFMv2 W_coef PhaseWave[0,LastFitPointLoc]  /X=VwireWave /W=ErrorWave /I=1 /R /E=EpsilonWave// /M=mask_g
	
	AppendFittedPolWaveVoltage2IFM(VwireWave, PhaseWave)				
...
Function CalculateConstantsVoltage()
	...
	Variable/G s = (L*hbar*2*pi)/(mass*a)								// beam separation at center of grad e region times velocity
End
...
Function Calc_EFieldVoltage2IFM(position)			// Calculates the electric field of the gradient region and makes waves for phase, prob, and contrast
	Variable position						// positions at which to calculate the E-field
	//Wave HVmonWave
	Nvar num_HV, d, E0overVwire, s		// d = distance from ground plane to infinitly thin effective wire
											// s = (beam separation)*velocity at center of GradE region. so s*vrecip = beam separation
	...
	// The electric field magnitude of the primary beam
	MatrixOP/O Etot= (E0overVwire) * sqrt(		magsqr(DplusYY/(magsqr(DplusYY)+xxsqr) + DminusYY/(magsqr(DminusYY)+xxsqr)) + magsqr(xx/(magsqr(DplusYY)+xxsqr) - xx/(magsqr(DminusYY)+xxsqr))   		)
	MatrixOP/O EtotMagSqr = magsqr(Etot*Vwire3d)
	
	
	//minus diffracted beam for the 0-1 IFM:
	MatrixOP/O DplusYsep = -1*s*vrecip+d+position//     d + position - (|s| * vrecip)
	MatrixOP/O DminusYsep =  s*vrecip - position + d 
	MatrixOP/O Etot_s= (E0overVwire)*sqrt(	magsqr(DplusYsep/(magsqr(DplusYsep)+xxsqr) + DminusYSep/(magsqr(DminusYSep)+xxsqr)) + magsqr(xx/(magsqr(DplusYsep)+xxsqr) - xx/(magsqr(DminusYsep)+xxsqr))		)
	MatrixOP/O Etot_sMagSqrM = magsqr(Etot_s*Vwire3d)
	
	
	//for the 0+1 IFM:
	s*=-1
	MatrixOP/O DplusYsep = -1*s*vrecip+d+position//d + position + (|s| * vrecip)
	MatrixOP/O DminusYsep =  s*vrecip - position + d 	// d - position - |s| * vrecip
	MatrixOP/O Etot_s= (E0overVwire)*sqrt(	magsqr(DplusYsep/(magsqr(DplusYsep)+xxsqr) + DminusYSep/(magsqr(DminusYSep)+xxsqr)) + magsqr(xx/(magsqr(DplusYsep)+xxsqr) - xx/(magsqr(DminusYsep)+xxsqr))		)
	MatrixOP/O Etot_sMagSqrP = magsqr(Etot_s*Vwire3d)
	
	// outputs of this function are the waves:      EtotMagSqr, Etot_sMagSqrM, Etot_sMagSqrP
End



Function CalcPhiOverPolVoltage2IFM()
	Wave EtotMagSqr, Etot_sMagSqrM, Etot_sMagSqrP, vrecip, IFMphase 
	...
	MatrixOP/O dphi_sM = Etot_sMagSqrM * vrecip					//phase shift of second path a distance "sep" away		// this is the 0th order
	MatrixOP/O dphi_sP = Etot_sMagSqrP * vrecip				//phase shift of third path a distance "sep" away		// this is the -1 order
...
Function PhiVoltage2IFM(pw, output, y) :FitFunc
	Wave pw, output, y									// pw = fit parameters; output = predicted phase shift; y = grad E position relative to beam
	
	Wave IFMprobM, IFMprobP, density, ContrastM, ContrastP, IFMphase_v_overpolM, IFMphase_v_overpolP
	...
	MatrixOP/O imagpartM = loc_v_step * density * sin(IFMphase_vM)			// weight imaginary part by prob(v) 
	MatrixOP/O realpartM = loc_v_step * density * cos(IFMphase_vM)				// weight real part by prob(v)
	Integrate/DIM=2 imagpartM, realpartM			// integrate real and imaginary parts along dv to find prob(v) weighted phase shift
	
	ContrastM = sqrt( magsqr(imagpartM[p][num_x-1][number_v-1]) + magsqr(realpartM[p][num_x-1][number_v-1]) )
	IFMprobM = atan2(imagpartM[p][num_x-1][number_v-1],realpartM[p][num_x-1][number_v-1])
	MatrixOP/O PhaseWrappedM = IFMprobM									// Duplicates IFMprob before unwrap for debugging
	
	Unwrap 2*Pi, IFMprobM		//Correction from taking the arctan.  
	if(IFMprobM[0]<0)
		IFMprobM+=2*pi
	endif
	...
	//repeat for 2nd IFM
	MatrixOP/O IFMphase_vP = pol * IFMphase_v_overpolP			// now we have the actual phase shift to take the real and imaginary parts of
											
	MatrixOP/O imagpartP = loc_v_step * density * sin(IFMphase_vP)			// weight imaginary part by prob(v) 
	MatrixOP/O realpartP = loc_v_step * density * cos(IFMphase_vP)				// weight real part by prob(v)
	Integrate/DIM=2 imagpartP, realpartP		// integrate real and imaginary parts along dv to find prob(v) weighted phase shift
	
	ContrastP = sqrt( magsqr(imagpartP[p][num_x-1][number_v-1]) + magsqr(realpartP[p][num_x-1][number_v-1]) )
	IFMprobP = atan2(imagpartP[p][num_x-1][number_v-1],realpartP[p][num_x-1][number_v-1])
	MatrixOP/O PhaseWrappedP = IFMprobP									// Duplicates IFMprob before unwrap for debugging
	
	Unwrap 2*Pi, IFMprobP		//Correction from taking the arctan.  
	if(IFMprobP[0]<0)
		IFMprobP+=2*pi
	endif
	
	output = (IFMprobM+IFMprobP)/2
End




Function PhiVoltage2IFMv2(pw, output, y) :FitFunc
	Wave pw, output, y									// pw = fit parameters; output = predicted phase shift; y = grad E position relative to beam
...
Function CleanupPolWavesVoltage()
	...
	KillWaves/Z imagpart, realpart, xx, xxsqr, yy, Vwire3d, v, vrecip, Etot_sMagSqrM, Etot_sMagSqrP
End
...
Function AppendFittedPolWaveVoltage2IFM(PositionWave, PhaseWave)
	Wave PositionWave, PhaseWave
	Wave W_coef
	

	PredictPhaseVoltage2IFM(W_coef[0])
